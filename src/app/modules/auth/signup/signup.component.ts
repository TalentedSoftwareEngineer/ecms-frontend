...
export class SignupComponent implements OnInit {
  ...
  get defValue() {
    ...
    return {
      ...
      color_scheme: this.notTest ? 'dark' : 'test',
  ...
  get getRegisterObj() {
    ...
    let obj = {
      ...
      'colorScheme': profile.color_scheme.value,
      "Customer": {
  ...
  getProducts() {
    ...
    this.api.getProductsReg().subscribe(res => {
      ...
      if (res) {
        ...
        setTimeout(() => {
          this.selectedProduct = products[0].value
          this.plan_sel = '$'+this.selectedProduct.price
        }, 100)
      }
      else{
  ...
  async request_email(reject) {
    return new Promise((resolve, reject) => {
      this.loading = true;
      this.api.request_email(this.getEmailForm.email.value).subscribe(res => {
        this.loading = false
  ...
  async verify_email(reject) {
    return new Promise(((resolve, reject) => {
      ...
      this.api.verify_email(this.getEmailForm.email.value, this.getCodeForm.code.value).subscribe(res => {
        this.loading = false;
        if (res.message && res.message === 'success' && res.data ) {
          this.stepComplete = 2
