...
export class NumberManComponent implements OnInit {
  ...
  getProviderNumberList(): Promise<void> {
    ...
    return new Promise(async resolve => {
      ...
      let filter = "limit=" + this.limit + "&type=" + this.selectedType.code.toLowerCase();

      if(this.selectedType.code.toLowerCase() == 'local' || this.selectedType.code.toLowerCase() == 'intl') {
        if(this.selectedCountry && this.selectedCountry.code.length > 0)
          filter += "&state=" + this.selectedCountry.code.toLowerCase();
      }

      if(this.selectedType.code.toLowerCase() == 'local') {
      ...
      } else if(this.selectedType.code.toLowerCase() == 'toll') {
        let number = "";
        let suffix = "";
        let lookup = "";
      ...
      this.api.getProviderNumbersList(filter).subscribe((data) => {
        if(data.length <= 0) {
          this.messageService.add({ key: 'tst', severity: 'warn', summary: 'Warning', detail: 'Provider returned no data for this lookup.' });
          return;
        }
        ...
        this.messageService.add({ key: 'tst', severity: 'error', summary: 'Error', detail: 'Provider error: ' + e });
      });
    });
  }
   ...
   */
   confirmPurchase() {
    ...
    let content: string =
      ...
      "      <th class=\"col-md-4\" style=\"text-align:right\"><h4>Order Total: $" + total.toString() + "/ month </h4> </th>" +
      "    </tr>" +
      "  </tfoot>" +
      " </table>" +
      " </div>" +
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        if(!blocked) {
          ...
          this.messageService.add({ key: 'tst', severity: 'warn', summary: 'Warning', detail: 'Insufficient Balance' });
        }
      },
      reject: () => {
        this.messageService.add({ key: 'tst', severity: 'warn', summary: 'Warning', detail: "Purchase cancelled." });
      },
      key: "purchaseDialog"
    });
  }
  ...
  formatUSPhoneNumber(str) {
   ...
   gotoNumberSetupPage() {
     this.messageService.add({ key: 'tst', severity: 'success', summary: 'Success', detail: 'Numbers correctly reserved.' });
  ...
  buyProviderNumbers(): Promise<void> {
    return new Promise(async resolve => {
      ...
      this.api.reserveProviderNumbers({numbers: JSON.stringify(this.selectedNumbers), customer: JSON.stringify(customer)}).subscribe((data) => {
        this.blockContent = false
        this.store.setReservedNumber(this.selectedNumbers);
...
//        this.clearForm();
        ...
        this.blockContent = false
        this.messageService.add({ key: 'tst', severity: 'error', summary: 'Error', detail: 'Unable to reserve numbers: ' + e });
      });
    });
  }

  onTypeChange(event) {
    let seltype = event.value;
    if(!seltype) {
    ...
    if(seltype.code == 'local') {

      //this.api.getCountries().subscribe((data) => {
      //  this.countries = data;
      //}, e => {
        this.countries = USCountries;
      //  this.toastr.error('Provider error: ' + e, '', {positionClass: 'toast-top-right'});
      //});

    } else if(seltype.code == 'toll') {
  ...
  onSelectedCountry(event) {
    ...
    if(this.selectedType && this.selectedType.code == 'local') {
  ...
  lookupButtonEvaluate() {
    if(
      (this.selectedType && this.selectedType.hasOwnProperty('code') && this.selectedType.code == "local" && this.selectedCountry && this.selectedCountry.hasOwnProperty('code') && this.selectedCountry.code.length > 0 && this.selectedRateCenter && this.selectedRateCenter.hasOwnProperty('rate_center') && this.selectedRateCenter.rate_center.length > 0) ||
      (this.selectedType && this.selectedType.hasOwnProperty('code') && this.selectedType.code == "intl" && this.selectedCountry && this.selectedCountry.hasOwnProperty('code') && this.selectedCountry.code.length > 0) ||
      (this.selectedType && this.selectedType.hasOwnProperty('code') && this.selectedType.code == "toll")
    ) {
  ...
  onImportNumbers = async (event: any, uploader: FileUpload) => {
    ...
    this.api.bulkNumberProvUpload({
      ...
      this.messageService.add({ key: 'tst', severity: 'success', summary: 'Success', detail: "File " + event.files[0].name + " correctly uploaded." });
    }, (e) => {
      this.messageService.add({ key: 'tst', severity: 'error', summary: 'Error', detail: "Error uploading file." });
    });
