...
import { callLogKeys, getFilter, getCountWhere, receivingNumberKeys, sipGatewayKeys, trackingKeys, userKeys, getParamsForFilter, callLogKeysForSupport } from '@app/helper/utils'
...
import { CallLog, CallLogSupport, CallRecording, LightLog, PhoneBook, LogsCount } from 'src/app/models/callLog'
...
export class ApiService {
  ...
  getUserCount(filterValue: string, customerFilter?: any): Observable<any> {
    const whereParam = getCountWhere(filterValue, null, null, null, customerFilter);
  ...
  downloadAllTrackingNumber(filterValue, customerFilter): Observable<any> {
    let whereParam = getCountWhere(filterValue, null, null, trackingKeys, customerFilter)
    return this.http.get<any>(`${this.coreApi}/OpNumbers/bulk_download?${'where=' + whereParam}`);
  }
  ...
  getSourcesCount(filterValue: string, customerFilter?: any): Observable<any> {
    ...
    return this.http.get<any>(`${this.coreApi}/TrackingSources/count?${'where=' + whereParam}`);
  }
   ...
   */
  // tslint:disable-next-line: max-line-length
  getLogs(active: string, direction: string, page: number, size: number, filterValue: string, date1: string, date2: string, option: string, value?: string): Observable<HttpResponse<CallLog[]>> {
   ...
   */
  // tslint:disable-next-line: max-line-length
  getTrackingNumbers(active: string, direction: string, page: number, size: number, filterValue: string, customerFilter?: any): Observable<HttpResponse<GetNumbers[]>> {
   ...
   */
  // tslint:disable-next-line: max-line-length
  getTrackingNumberCount(filterValue: string, customerFilter?: any): Observable<any> {
    const whereParam = getCountWhere(filterValue, null, null, trackingKeys, customerFilter);
    return this.http.get<any>(`${this.coreApi}/OpNumbers/count?${'where=' + whereParam}`);
  }
  ...
  /**
   * Get Call Logs Count for dashboard
   * @param dateFrom
   * @param dateTo
   */
  getDashLogsCount(dateFrom: string, dateTo: string): Observable<LogsCount> {
    let created = { created: { between: [dateFrom, dateTo] } }
    let where = {}
    ...
    // let whereParam = encodeURIComponent(JSON.stringify(created))

    return this.http.get<LogsCount>(`${this.coreApi}/CallLogs/count_logs?${'where=' + whereParam}`);
  }
  ...
  getReceivingNumbers(active: string, direction: string, page: number, size: number, filterValue: string, customerFilter?: any): Observable<HttpResponse<ReceivingNumber[]>> {
    // tslint:disable-next-line: max-line-length
    const filter = getFilter(active, direction, size, page, filterValue, null, null, receivingNumberKeys, null, customerFilter);
    // tslint:disable-next-line: max-line-length
  ...
  activityReport(startDate, endDate, interval, viewBy, offset, customerId): Observable<any> {
    const body = new HttpParams()
  ...
  overviewReport(startDate, endDate, offset, interval, customerId): Observable<any> {
    const body = new HttpParams()
      .set('start_date', startDate)
      .set('end_date', endDate)
      .set('offset', offset)
  ...
  /**
   * Get Call Logs from new backend
   * @param filterValue
   * @param date1
   * @param date2
   */
  // tslint:disable-next-line: max-line-length
  getLogsCountFromNewBackend(filterValue: string, date1: string, date2: string, customerId: string): Observable<any> {

    const params = getParamsForFilter(filterValue);
    const body = new HttpParams()
      ...
      .set('customer_id', customerId)
      .set('created', params.created);

    // tslint:disable-next-line: max-line-length
    return this.http.post(`${this.coreApi}/CallLogs/countForNewServer`, body.toString(), { headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded') });
  }
   ...
   * @returns
   */
  getLogsCount(filterValue: string, date1: string, date2: string, customerFilter: any): Observable<any> {
    const whereParam = getCountWhere(filterValue, date1, date2, callLogKeys, customerFilter);
   ...
   */
  // tslint:disable-next-line: max-line-length
  getLogsFromNewBackend(active: string, direction: string, page: number, size: number, filterValue: string, date1: string, date2: string, customerId: string, option: string, value?: string): Observable<any> {

    const params = getParamsForFilter(filterValue);

    // const body = new HttpParams().set('filter', filter)
    const body = new HttpParams().set('start_date', date1)
      .set('end_date', date2)
      .set('search', params.search)
      .set('fields', params.fields)
      .set('customer_id', customerId)
    ...
    // tslint:disable-next-line: max-line-length
    return this.http.post(`${this.coreApi}/CallLogs/new`, body.toString(), { headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded') });
  }

  /**
   * get csv data from backend for call logs
   * @param active
   * @param direction
   * @param page
   * @param size
   ...
   * @param value
   */
  getCallLogExport(active: string, direction: string, page: number, size: number, filterValue: string, date1: string, date2: string, customerId: string, option: string, value?: string): Observable<any> {
    ...
    // tslint:disable-next-line: max-line-length
    return this.http.post(`${this.coreApi}/CallLogs/export`, body.toString(), { headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded') });
  }
  ...
  getTransactions(id): Observable<any> {
    return this.http.get(`${this.coreApi}/PaymentTransactions?filter={"order": ["transactionDate DESC"], "where":{"customerId":${id}}}`, {});
  }
   ...
   * @returns
   */
  request_email(email_address): Observable<any> {
    return this.http.post(`${this.coreApi}/DashUsers/request_email`, { email_address: email_address })
  }
